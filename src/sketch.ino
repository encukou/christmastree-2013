...
void loop() {
    ...
    for (int i=0; i<NUM_LEDS; i++) {
        if (intensity[i]) {
            int r = (int)intensity[i] * ((int)random(55) + 200) / 255 * x(vr(hue[i])) / xmax;
            int g = (int)intensity[i] * ((int)random(55) + 200) / 255 * x(vg(hue[i])) / xmax;
            int b = (int)intensity[i] * ((int)random(55) + 200) / 255 * x(vb(hue[i])) / xmax;
